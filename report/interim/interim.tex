...
Better measures are Precision and Recall scores. The Precision measures the proportion of predictions that were successfully correct. The Recall, on the other hand, is measuring the proportion of predictions where actual positives were identified correctly. These scores can be further combined into a more general score $F_\beta$.

\begin{equation*}
  \text{Precision} = \frac{TP}{TP+FP}, \quad \text{Recall} = \frac{TP}{TP+FN}, \quad \text{F}_\beta = (1+\beta^2)\cdot \frac{\text{Precision} \cdot \text{Recall}}{(\beta^2+\text{Precision})+\text{Recall}}
\end{equation*}

\begin{warning}
  Sources used: \textit{\href{https://developers.google.com/machine-learning/crash-course/classification/precision-and-recall}{Google MML crash course}}
\end{warning}

\section{Overlap Based}
...
\section{Honorable mentions}\label{sect:evaluation-metrics-honorable-mentions}
